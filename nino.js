		...
		message: {
			"productMessage": {
				"product": {
					...
					"title": 'Apicca Bot々', 
					"description": "Apicca Bot", 
					"currencyCode": "IDR",
					"priceAmount1000": "99999999",
					"retailerId": 'Whatsapp Bot',
					"productImageCount": 1
				},
				    "businessOwnerJid": `6285861870154@s.whatsapp.net`
		}
      ...
      mentions(cptl, x, true)
       ...
       // CMD
        ...
        case 'creator':
               ...
               buttons = [{buttonId: '!sc',buttonText:{displayText: 'SC BOT'},type:1},{buttonId:'!ytbot',buttonText:{displayText:'YOUTUBE'},type:1}]

               imageMsg = ( await nino.prepareMessage(from, fs.readFileSync(`./media/Menu.jpg`), 'imageMessage')).message.imageMessage

               buttonsMessage = {
               contentText: `${txtt}`,
               footerText: 'Jangan sungkan chat ya kak', imageMessage: imageMsg,
               buttons: buttons,
               headerType: 4
}
               ...
               break
               case 'id':
               let thumbnail = await getBuffer('https://justrafa.herokuapp.com/api/thumbnail?apikey=Apicca')
                 thu = await nino.getStatus(`${sender.split('@')[0]}@s.whatsapp.net`, MessageType.text)
totalChat = await nino.chats.all()
ultah = await fetchJson(`https://api-ramlan.herokuapp.com/api/hitungmundur?tanggal=12&bulan=10`)

               menu =`Hai ${pushname} 👋

*Social Media :*
• Github : MurphyBot
• Youtube : Apicca Official
• _Rest Api_ : justrafa.herokuapp.com
...
*Note :*
• Jika kebingungan, kamu dapat membaca tata cara pakai ketik command
• Contoh : ${prefix}infomenu nanti akan muncul menu dan cara pakai`

 nino.sendMessage(from, { contentText: `${menu}`, footerText: '© By Rafasurya', buttons: [{ buttonId:  `!update`, buttonText: { displayText: 'Changelog' }, type: 1 }], headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: thumbnail }}, 'buttonsMessage')
break
case 'help':
case 'menu':
        let buf = await getBuffer('https://justrafa.herokuapp.com/api/loli?apikey=Apicca')
               parseMention = (text = '') => {
...
࿊ _Rest Api_ : justrafa.herokuapp.com`                          
               buttons = [{buttonId: `${prefix}id`,buttonText:{displayText: 'Menu'},type:1},{buttonId:`${prefix}allmenu`,buttonText:{displayText:'Menu 2'},type:1},{buttonId: `${prefix}y`,buttonText:{displayText: 'Syarat & Ketentuan'},type:1}]

               imageMsg = ( await nino.prepareMessage(from, await getBuffer('https://justrafa.herokuapp.com/api/loli?apikey=Apicca'), 'imageMessage')).message.imageMessage

               buttonsMessage = {
               contentText: `${menu}`,
               footerText: `© By Rafasurya | ${totalChat.length} registered users`, imageMessage: imageMsg,
               buttons: buttons,
               headerType: 4
}
            
               prep = await nino.prepareMessageFromContent(from,{buttonsMessage},{quoted: mek, contextInfo:{ mentionedJid: parseMention(menu), externalAdReply: {
							title: `Hai ${pushname} 👋`,
							body: `Suka sama scriptnya? Beli murah kok`,
							previewType: 1,
							thumbnailUrl: "https://telegra.ph/file/80c37eef047bb0bd98446.jpg",
							thumbnail: await getBuffer('https://telegra.ph/file/80c37eef047bb0bd98446.jpg'), }, forwardingScore:508, isForwarded:true, mentionedJid:[senderr]}})
                            nino.relayWAMessage(prep)
                            break                     


        case 'su':
         textImg(`*REST API :* justrafa.herokuapp.com\n*Pricing :* pricing.herokuapp.com`)
         break
        case 'command':
               list = []
               ...
               listmsg(from, `${ucapanWaktu}`, `Hai kak ${pushname}, Pilih Menu ApiccaBot Disini`, list)
               break
//------------------< Game >------------------- 
        case 'limitgame': 
        case 'balance': 
               const balance = atm.checkATMuser(sender, _uang)
               if (isPremium) return reply(`Limit : Unlimited\nBalance : Rp.${balance}`)
               textImg(`Limit Game : ${cekGLimit(sender, gcount, glimit)}/${gcount}\nBalance : Rp.${balance}`)
               break
         case 'gelud':
         if (isGame(sender, isPremium, isOwner, gcount, glimit)) return reply(`Limit game kamu sudah habis`)
               if (isGame(sender, isPremium, isOwner, gcount, glimit)) return reply(`Limit game kamu sudah habis`)
...
🎁 Type : mp3/mp4
🚀 Durasi : ${res[0].size}`
              buttons = [{buttonId: `${prefix}buttons2 ${q}`,buttonText:{displayText: `Video`},type:1},{buttonId:`${prefix}ok1 ${q}`,buttonText:{displayText:'Audio'},type:1}]
...
case 'tiktok':
          ...
          case 'tiktokmusic':
              ...
              buttons = [{buttonId: `${prefix}wm ${q}`,buttonText:{displayText: `With Watermark`},type:1},{buttonId:`${prefix}buttons3 ${q}`,buttonText:{displayText:'No Watermark'},type:1},{buttonId:`${prefix}buttons4 ${q}`,buttonText:{displayText:'Music'},type:1}]
              ...
              buttonsMessage = {footerText:'© By Murphy', imageMessage: imageMsg,
              contentText:`${result}`,buttons,headerType:4}
              prep = await nino.prepareMessageFromContent(from,{buttonsMessage},{quoted: mek})
              nino.relayWAMessage(prep)
              ...
              case 'ytmp3':
              if (!q) return reply('Linknya?')
               data = await fetchJson(`https://justrafa.herokuapp.com/api/yutub/audio?url=${q}&apikey=Apicca`)
              result = `*「 YOUTUBE AUDIO 」*\n\n*Title :* ${data.result.title}`
              buttons = [{buttonId: `!ok1 ${q}`,buttonText:{displayText: `${data.result.filesize}`},type:1}]
              fs.writeFileSync(`./${sender}.jpeg`, await getBuffer(data.result.thumb))
              imageMsg = ( await nino.prepareMessage(from, fs.readFileSync(`./${sender}.jpeg`), 'imageMessage', {thumbnail: Buffer.alloc(0)})).message.imageMessage
              buttonsMessage = {footerText:'© By Rafasurya', imageMessage: imageMsg,
              contentText:`${result}`,buttons,headerType:4}
              prep = await nino.prepareMessageFromContent(from,{buttonsMessage},{quoted: mek})
              nino.relayWAMessage(prep)
              fs.unlinkSync(`./${sender}.jpeg`)
              ...
              case 'ok1':
               data = await fetchJson(`https://justrafa.herokuapp.com/api/yutub/audio?url=${q}&apikey=Apicca`)
              ini_wm = await getBuffer(data.result.result)
              nino.sendMessage(from, ini_wm, audio, { quoted: mek })
              break
              case 'ok':
               data = await fetchJson(`https://api.zeks.me/api/ytplaymp4?apikey=Apicca-key&q=${q}`)
              ini_wm = await getBuffer(data.result.url_video)
              nino.sendMessage(from, ini_wm, video, { quoted: mek })
              break
              case 'ytmp4':
              if (!q) return reply('Linknya?')
               data = await fetchJson(`https://justrafa.herokuapp.com/api/yutub/video?url=${q}&apikey=Apicca`)
              result = `*「 YOUTUBE VIDEO 」*\n\n*Title :* ${data.result.title}`
              buttons = [{buttonId: `!ok ${q}`,buttonText:{displayText: `${data.result.resolution} (${data.result.filesize})`},type:1}]
              fs.writeFileSync(`./${sender}.jpeg`, await getBuffer(data.result.thumb))
              imageMsg = ( await nino.prepareMessage(from, fs.readFileSync(`./${sender}.jpeg`), 'imageMessage', {thumbnail: Buffer.alloc(0)})).message.imageMessage
              buttonsMessage = {footerText:'© By Rafasurya', imageMessage: imageMsg,
              contentText:`${result}`,buttons,headerType:4}
             ...
             data = await fetchJson(`https://api.zeks.me/api/ytplaymp4?apikey=Apicca-key&q=${q}`)
                res = await fetchJson(`https://justrafa.herokuapp.com/api/yutub/audio?url=${data.result.source}&apikey=Apicca`)
              result = `*「 YOUTUBE PLAY 」*\n\n*Title :* ${data.result.title}`
              buttons = [{buttonId: `!ok ${q}`,buttonText:{displayText: `Video 360p (${data.result.size})`},type:1},{buttonId: `!play1 ${data.result.source}`,buttonText:{displayText: `Audio (${res.result.filesize})`},type:1}]
              fs.writeFileSync(`./${sender}.jpeg`, await getBuffer(data.result.thumbnail))
              imageMsg = ( await nino.prepareMessage(from, fs.readFileSync(`./${sender}.jpeg`), 'imageMessage', {thumbnail: Buffer.alloc(0)})).message.imageMessage
              buttonsMessage = {footerText:'© By Rafasurya', imageMessage: imageMsg,
              contentText:`${result}`,buttons,headerType:4}
          ...
          case 'wm':
              if (!q) return reply('Linknya?')
              if (!q.includes('tiktok')) return reply(mess.error.Iv)
              data = await fetchJson(`https://justrafa.herokuapp.com/api/tiktok/ttdownloader?url=${q}&apikey=Apicca`)
              ini_wm = await getBuffer(data.result.wm)
              nino.sendMessage(from, ini_wm, video, { quoted: mek })
              break
          case 'buttons3':
              if (!q) return reply('Linknya?')
              if (!q.includes('tiktok')) return reply(mess.error.Iv)
              data = await fetchJson(`https://justrafa.herokuapp.com/api/tiktok/ttdownloader?url=https://vm.tiktok.com/ZSJwaYt7d/&apikey=Apicca`)
          ...
          case 'buttons4': 
              if (!q) return reply('Linknya?')
              if (!q.includes('tiktok')) return reply(mess.error.Iv)
              data = await fetchJson(`https://justrafa.herokuapp.com/api/tiktok/ttdownloader?url=${args[0]}&apikey=Apicca`)
              ini_audio = await getBuffer(data.result.audio)
              nino.sendMessage(from, ini_audio, audio, { quoted: mek })
              break
          case 'buttons5':
      ...
      case 'kusonime':
       ...
       case 'hentai':
       case 'memeindo':
              let wipu = (await axios.get(`https://raw.githubusercontent.com/MurphyBot/datayo/main/${command}.json`)).data
       ...
       case '3dnekopoilast':
        ...
        case 'otakuongoing':              
               ...
               break
                  case 'play1':
              await axios.get(`https://justrafa.herokuapp.com/api/yutub/audio?url=https://youtu.be/v0ej6EJGqos&apikey=Apicca`)
		     .then(res => {
			  nino.sendMessage(from, { url: res.data.result.result }, 'audioMessage', { mimetype: 'audio/mp4', quoted: mek, contextInfo: { externalAdReply: { title: res.data.result.title, mediaType: 2, thumbnailUrl: res.data.result.thumb, mediaUrl: res.data.result.result }}})
})
              break
          case 'lirik':
               if (!q) return reply(mess.wrongFormat)
...
`
          ...
          case 'pin':
              ...
              buttons = [{buttonId: `${prefix + command} ${q}`,buttonText:{displayText: `➡️Next`},type:1},{buttonId:`y`,buttonText:{displayText:'🚮List Image'},type:1}]
...
\`\`\`▢ Link : ${i.url}\`\`\``
       ...
       case 'img2url':
               ...
               imgbb('https://api.imgbb.com/1/upload?expiration=120&key=02320582194520b4367775b536873e34&image=${media}&name=Rafasurya')
              .then(data => {
...
//------------------< Owner >-------------------
      ...
      case 'update':
      case 'infoupdate':
             let updateList = `*── 「 UPDATE APICCA BOT 」 ──*\n\n\n`
             for (let i of _update) {
             updateList += `࿃ *Title :* Update V.1.0.0\n࿃ *Terakhir Update :* Minggu 20 September\n*Result :*\n\n࿃ Add Info Update (!update)\n\nFix Tiktok ${i.replace(_update)}\n\n`
}
             textImg(updateList)
             break
      case 'reset':
...
//------------------< Lainnya >-------------------
        ...
        case 'info':  // Jangan Di Ubah Plise
               urlinfo = 'https://k.top4top.io/p_2045sg8hc1.jpg'
